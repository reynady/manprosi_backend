...
export default function makeRecommendationsRouter(db){
  ...
  router.post('/', [
    ...
    try{
      ...
      // include created_by only if column exists (best-effort)
      try{
        const [cols] = await db.execute("SELECT COUNT(*) AS cnt FROM information_schema.columns WHERE table_schema = ? AND table_name = 'recommendations' AND column_name = 'created_by'", [process.env.DB_NAME || 'palm_oil_monitoring']);
        if(cols[0].cnt > 0) payload.created_by = req.user.id;
      }catch(_){ /* ignore */ }
  ...
  router.put('/:id', [
    ...
    try{
      ...
      const roleName = (req.user.role||'').toLowerCase();
      const hasCreatedByRow = await db.execute("SELECT COUNT(*) AS cnt FROM information_schema.columns WHERE table_schema = ? AND table_name = 'recommendations' AND column_name = 'created_by'", [process.env.DB_NAME || 'palm_oil_monitoring']);
      const hasCreatedBy = hasCreatedByRow[0][0].cnt > 0;
  ...
  router.delete('/:id', async (req,res)=>{
    try{
      ...
      if(req.user.user_role_id !== 1 && roleName !== 'consultant'){
        if(hasCreatedBy){ if(existing.created_by !== req.user.id) return res.status(403).json({ success:false, error:'Forbidden' }); }
        else return res.status(403).json({ success:false, error:'Forbidden' });
      }
